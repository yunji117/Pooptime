...
function MyRecommend() {
  ...
  const fetchRecommends = async () => {
    try {
      const response = await fetch("http://localhost:8080/community/user/recommends", {
        credentials: "include",
      });
      ...
      console.error("추천 컨텐츠 조회 실패", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const removeItem = async (id, boardId) => {
    try {
      await fetch(`http://localhost:8080/community/recommend/${boardId}`, {
        method: "DELETE",
        credentials: "include",
      });
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex items-center pb-3 mb-2">
        ...
        <div className="ml-4">
          <h4 className="text-base font-semibold text-(--text-900)">내가 추천한 컨텐츠</h4>
          <p className="text-xs text-(--text-600)">추천한 컨텐츠를 한눈에 확인하세요.</p>
        </div>
      </div>
      ...
      <div className="bg-(--surface-0) rounded-xl p-4 shadow-[0_0_5px_0_rgba(0,0,0,0.08)]">
        {loading ? (
          ...
          <p className="text-sm text-(--text-600)">추천한 컨텐츠가 없습니다.</p>
        ) : (
